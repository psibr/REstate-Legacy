...
namespace REstate.Repositories.Core.Susanoo
{
        ...
        : ConfigurationContextualRepository, IMachineConfigurationRepository
    {
        ...
        public async Task<IStateMachineConfiguration> RetrieveMachineConfiguration(string machineDefinitionId, bool loadCode,
            CancellationToken cancellationToken)
        {
                ...
                new[]
            {
                ...
                typeof (REstate.Configuration.State),
                typeof (REstate.Configuration.Trigger),
                typeof (Transition),
                typeof (IgnoreRule),
                typeof (Guard),
                typeof (StateAction),
            ...
            var codeElements = loadCode ? results[7].Cast<ICodeWithDatabaseConfiguration>().ToList()
                : null;
        ...
        public async Task<IStateMachineConfiguration> RetrieveMachineConfigurationForInstance(string machineInstanceId, bool loadCode,
            CancellationToken cancellationToken)
        {
            var machineDefinitionId = await CommandManager.Instance
                    .DefineCommand(
                        "SELECT TOP 1 MachineDefinitionId FROM MachineInstances WHERE MachineInstanceId = @MachineInstanceId",
                        CommandType.Text)
                    .Realize()
                    .ExecuteScalarAsync<string>(DatabaseManagerPool.DatabaseManager, new { MachineInstanceId = machineInstanceId },
                        cancellationToken);

            if (machineDefinitionId == null)
                throw new ArgumentException("Not a valid instance identifier.", nameof(machineDefinitionId));

            var configuration = await RetrieveMachineConfiguration(machineDefinitionId, loadCode, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            var newTriggers = (await CommandManager.Instance
                ...
                .DefineResults<REstate.Configuration.Trigger>()
                .Realize()
                .ExecuteAsync(DatabaseManagerPool.DatabaseManager,
        ...
        public async Task<IMachineDefinition> SetInitialState(string machineDefinitionId,
        ...
        public async Task<ITransition> UpdateTransition(ITransition transition, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(transition.MachineDefinitionId))
        ...
        public async Task<IGuard> UpdateGuard(IGuard guard, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(guard.MachineDefinitionId))
                throw new ArgumentException("MachineDefinitionId is a required property.", nameof(guard));
        ...
        public async Task<IMachineDefinition> ToggleMachineDefinitionActive(string machineDefinitionId, bool isActive,
            CancellationToken cancellationToken)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(machineDefinition.MachineDefinitionId))
            ...
            CancellationToken cancellationToken)
        {
            ...
            var newId = newDefinition.MachineDefinitionId;
            ...
            var newConfiguration = await RetrieveMachineConfiguration(newId, false, cancellationToken);
